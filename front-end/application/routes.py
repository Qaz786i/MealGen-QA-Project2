...
@app.route('/')
def index():
    main = requests.get('http://service_2:5000/get-mains').json()['main']
    side = requests.get('http://service_3:5000/get-side').json()['side']
    prices = requests.post('http://service_4:5000/post-meal', json={"main":main, "side":side})
    ...
    db.session.commit()
    meals = Make_meal.query.order_by(Make_meal.id.desc()).limit(1).all()
